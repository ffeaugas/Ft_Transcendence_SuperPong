...
export class Home extends Phaser.Scene {
  ...
  create() {
    ...
    this.ballEntity[1] = this.add.image(400, 300, "particule").setScale(0.2);
  ...
  update(time: number, delta: number): void {
    ...
    if (this.ballEntity[1].x <= 22.5) {
      this.playerTurn = 0;
      this.ballAngle = (this.ballEntity[1].y - this.playerEntities[0].y) / 7;
    }
    if (this.ballEntity[1].x >= 777.5) {
      this.playerTurn = 1;
      this.ballAngle = (this.ballEntity[1].y - this.playerEntities[1].y) / 7;
    }
    if (this.playerTurn == 1) {
      ...
      if (
        this.ballEntity[0].y < 0 ||
        this.ballEntity[0].y > 600 ||
        this.ballEntity[0].x < 0 ||
        this.ballEntity[0].x > 800
      ) {
        this.ballEntity[0].x = 400;
        this.ballEntity[0].y = 300;
        this.ballAngle = Math.random() % 5;
        this.ballDirection = 10;
      }
    }
  }
}
