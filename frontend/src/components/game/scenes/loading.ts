...
export default class LoadingScene extends Phaser.Scene {
    ...
    async getProfileDatas(player: string): Promise<any> {
        try {
        ...
        this.room.state.players.onAdd((player, sessionId) => {
            ...
            this.room.onMessage("Joined", (player) => {
                if (!this.playerSessionID[0] && this.name[1] != player) {
                    const profileData = this.getProfileDatas(player).then(
                        (profileData) => {
                            const img = this.load.image(
                                "p1",
                                `http://${process.env.NEXT_PUBLIC_DOMAIN}:3001/uploads/avatar/${profileData["profilePicture"]}`
                            );
                            this.add.image(30, 300, "p1").setScale(0.5);
                            this.load.start();
                        }
                    );
                    ...
                    const profileData = this.getProfileDatas(player).then(
                        (profileData) => {
                            ...
                            this.load.start();
                            this.add.image(500, 10, "p2").setScale(0.5);
                        }
                    );
